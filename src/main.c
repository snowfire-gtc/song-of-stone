...
Camera2D g_camera = {0};
bool g_camera_free_mode = false;  // true = свободная камера, false = слежение за игроком
Vector2 g_camera_free_position = {0};  // позиция свободной камеры
...
int main(void) {
    ...
    // Инициализация позиции свободной камеры
    g_camera_free_position = (Vector2){world->throne_blue.x, world->throne_blue.y};
    ...
    while (!WindowShouldClose()) {
        ...
        switch (game_state) {
            ...
            case GAME_STATE_PLAYING:
                ...
                // Переключение режима камеры на 'C'
                if (IsKeyPressed(KEY_C)) {
                    g_camera_free_mode = !g_camera_free_mode;
                    if (g_camera_free_mode) {
                        // При включении свободного режима запоминаем текущую позицию
                        g_camera_free_position = g_camera.target;
                    } else {
                        // При выключении свободного режима возвращаем камеру к игроку
                        if (world->char_count > 0) {
                            Character* player = &world->characters[world->local_player_id];
                            g_camera.target.x = player->x + 8;
